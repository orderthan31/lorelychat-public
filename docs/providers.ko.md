...
`LLM_MOCK=true`는 설치 검증용 모드입니다. 프로바이더 계정이 필요하지 않으며 유료 생성 요청도 보내지 않습니다.
...
6. 런타임 기본 모델을 지정합니다.
7. Mock 모드를 끄고 API 컨테이너를 다시 만듭니다.

프로바이더 키는 `/secrets/provider_secrets.json`에 평문 JSON 값으로 저장되며, 데이터베이스 레코드에서는 불투명 ID로 참조합니다. POSIX 권한을 지원하는 파일시스템에서는 파일 권한 `0600`을 사용합니다. Lorechat은 API 응답에서 키를 가리지만 비밀정보 파일 자체를 암호화하지는 않습니다.

## 로컬 OpenAI-compatible 엔드포인트

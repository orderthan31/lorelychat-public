...
처음 실행하면 합성 데모 캐릭터와 세계관이 생성됩니다. 런타임 데이터는 Docker named volume에 저장됩니다.
...
중요한 볼륨에는 데이터, 업로드, 로그 및 비밀정보가 들어 있습니다. 호스트의 백업 도구나 검토된 임시 컨테이너로 이를 보관하고, 백업은 암호화해 저장소 밖에 두세요.
...
`docker compose down`은 데이터를 보존합니다. Named volume을 제거하면 데이터베이스, 업로드, 로그 및 로컬에 저장된 프로바이더 키가 영구 삭제됩니다. 삭제가 의도된 작업이고 백업도 완료된 경우가 아니라면 볼륨 제거 명령을 실행하지 마세요.
